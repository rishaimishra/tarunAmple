    ...
    */
    ...
    if(isset($_POST['page']) && !empty($_POST['page']))
    {
        ...
        elseif($_POST['page'] == "remove_this_item") //Remove a specific item from the cart
        {
            ...
            if(!empty($uskid)) {
                $vpb_check_appliedamples = mysqli_query($con,"select apply_amples as `user_applied_amples` from `products_added` where `product_id` = '".mysqli_real_escape_string($con,strip_tags($_POST["item_id"]))."' and `customer_Id` = '".mysqli_real_escape_string($con,$uskid)."' and `is_purchased` = 0");
            ...
            if(!empty($uskid)) {
                mysqli_query($con,"delete from `products_added` where `product_id` = '".mysqli_real_escape_string($con,strip_tags($_POST["item_id"]))."' and `customer_Id` = '".mysqli_real_escape_string($con,$uskid)."' and `is_purchased` = 0");
                ...
                mysqli_query($con,"delete from `products_added` where `product_id` = '".mysqli_real_escape_string($con,strip_tags($_POST["item_id"]))."' and `username` = '".mysqli_real_escape_string($con,$_SESSION["REMOTE_ADDR"])."' and `is_purchased` = 0");
                    ...
                    <li class="product-info">
                        <div class="p-left">
                            <a href="<?php echo $baseurl.'/productdetail/'.$key['pid']; ?>">
                                <img class="img-responsive" src="<?php echo cdnUrl('product_images/'.$key['pid'].'/'.$vpb_get_imagedata['product_image']); ?>" alt="p10">
                            </a>
                        </div>
                        <div class="p-right">
                            ...
                            <a onclick="remove_this_item('<?php echo $key['pid']; ?>', '<?php echo $uskid; ?>');" href="javascript:void(0);" class="remove_link">Remove</a>
                        </div>
                    </li>
                    <?php } } ?>
                ...
                if(mysqli_num_rows($vpb_check_itemsdata)>0) {
                    ...
                    <tr>
                        <td class="cart_product">
                            <a href="<?php echo $baseurl.'/productdetail/'.$key['pid']; ?>"><img src="<?php echo cdnUrl('product_images/'.$key['pid'].'/'.$vpb_get_imagedata['product_image']); ?>" alt="Product"></a>
                        </td>
                        <td class="cart_description">
                            <p class="product-name"><a href="<?php echo $baseurl.'/index/productdetail/pid/'.''.$key['pid']; ?>"><?php echo $key['item_name']; ?> </a></p>
                            <small class="cart_ref">SKU : #123654999</small><br>
                            <small>Color : <div style="margin:-21px 0px 0px 50px;width:20px;height:20px;background-color:<?php echo $key['cacolor']; ?>;"></div></small>  
                        ...
                        <td class="cart_pickup_delivery">
                            <strong>&nbsp; &nbsp; &nbsp; &nbsp; <input type="radio" id="vdrpickupdivadr<?php echo $keycount; ?>" value="vdrpickupadr<?php echo $keycount; ?>" onclick="openvendoraddress(this.value,document.getElementById('cstdelverdivadr<?php echo $keycount; ?>').value);" name="pickdelivery<?php echo $keycount; ?>"></input>Pickup &nbsp; &nbsp; &nbsp; <input type="radio" id="cstdelverdivadr<?php echo $keycount; ?>" value="cstdelveradr<?php echo $keycount; ?>" onclick="opencustomeraddress(this.value,document.getElementById('vdrpickupdivadr<?php echo $keycount; ?>').value);" name="pickdelivery<?php echo $keycount; ?>"></input>Delivery</strong> <br />
                            <div id="vdrpickupadr<?php echo $keycount; ?>" style="display:none;"><?php if(mysqli_num_rows($vpb_check_itemsdata)>0) { while($vendoradrkey = mysqli_fetch_assoc($vpb_check_vendoraddress)) { if($vendoradrkey['adr_vdr_id'] == $key['vadrid']) { ?><input type="radio" name="vendor_address<?php echo $keycount; ?>"> <?php echo $vendoradrkey['adr_street_line1'].", ".$vendoradrkey['adr_city'].", ".$vendoradrkey['adr_state'].", ".$vendoradrkey['adr_country']."-".$vendoradrkey['adr_zip']; ?> </input> <?php echo "<br />"; } } } ?></div>
                            <div id="cstdelveradr<?php echo $keycount; ?>" style="display:none;"><?php if(!empty($vpb_get_useraddress['address']) && !empty($vpb_get_useraddress['user_city']) && !empty($vpb_get_useraddress['user_state']) && !empty($vpb_get_useraddress['zip_code'])) { ?><input type="radio" name="vendor_address<?php echo $keycount; ?>"> <?php echo $vpb_get_useraddress['address'].", ".$vpb_get_useraddress['user_city'].", ".$vpb_get_useraddress['user_state'].", ".$vpb_get_useraddress['user_country']."-".$vpb_get_useraddress['zip_code']; ?></input><?php } else { echo "Sorry! You don't have a complete address information within your account."; } ?></div>
                        </td>
                        <td class="price"><span>$ <?php if((strip_tags($key['apply_amples']) == 0.00) && (strip_tags($key['newprice_byamples']) == 0.00)) { echo $key['item_single_amount']; } else { echo strip_tags($key['newprice_byamples']); } ?></span></td>
                        <td class="qty">
                            <input class="form-control input-sm" type="text" value="<?php echo $key['item_quant']; ?>" disabled readonly />
                            <?php if((strip_tags($key['apply_amples']) == 0.00) && (strip_tags($key['newprice_byamples']) == 0.00)) { ?>
                                <a href="javascript:void(0);" onclick="vpb_add_to_cart('<?php echo strip_tags($key['item_name']); ?>','<?php echo strip_tags($key['pid']); ?>','<?php echo strip_tags($vpb_get_productdata['min_order_quantity']); ?>','<?php echo strip_tags($key['item_single_amount']); ?>','<?php if(!empty($key['earned_amples'])) { echo $key['earned_amples']; } else { echo "0.00"; } ?>','<?php if(!empty($vpb_get_useraddress['total_ample'])) { echo $vpb_get_useraddress['total_ample']; } else { echo "0.00"; } ?>','<?php echo $uskid; ?>','<?php echo strip_tags($key['vadrid']); ?>','add');"><i class="fa fa-caret-up"></i></a>
                                <a href="javascript:void(0);" onclick="vpb_dcre_from_cart('<?php echo strip_tags($key['item_name']); ?>','<?php echo strip_tags($key['pid']); ?>','<?php echo strip_tags($vpb_get_productdata['min_order_quantity']); ?>','<?php echo strip_tags($key['item_single_amount']); ?>','<?php echo $uskid; ?>','del');"><i class="fa fa-caret-down"></i></a>
                                <?php } ?>
                        </td>
                        <td class="price">
                        ...
                        <td class="action">
                            <a href="javascript:void(0);" onclick="remove_this_item('<?php echo $key['pid']; ?>', '<?php echo $uskid; ?>');" >Delete item</a>
                        </td>
                    </tr>            


                    <?php $keycount++; } } else { echo "<tr> <td> No Item Available in My Cart </td> </tr>"; } ?>
        ...
        else if($_POST['page'] == "decrement_this_item") //Remove a specific item from the cart
            {
                ...
                $uskid   =  $_POST['usrmaid'];
                if(!empty($uskid)) {
                    $vpb_check_items = mysqli_query($con,"select * from `products_added` where `customer_Id` = '".mysqli_real_escape_string($con,$uskid)."' and `item_added` = '".mysqli_real_escape_string($con,$_POST['item_name'])."' and `is_purchased` = 0");
                }
                else {
                    $vpb_check_items = mysqli_query($con,"select * from `products_added` where `username` = '".mysqli_real_escape_string($con,$_SESSION["REMOTE_ADDR"])."' and `item_added` = '".mysqli_real_escape_string($con,$_POST['item_name'])."' and `is_purchased` = 0");
                }
            ...
            <?php 
                ...
                if(mysqli_num_rows($vpb_check_itemsdata)) {
                    ...
                    <tr>
                        ...
                        <td class="cart_description">
                            <p class="product-name"><a href="<?php echo $baseurl.'/productdetail/'.$key['pid']; ?>"><?php echo $key['item_name']; ?> </a></p>
                            <small class="cart_ref">SKU : #123654999</small><br>
                            <small>Color : <div style="margin:-21px 0px 0px 50px;width:20px;height:20px;background-color:<?php echo $key['cacolor']; ?>;"></div></small>  
                            <small>Size : <?php echo $key['casize']; ?></small>
                        </td>
                        <td class="cart_avail"><span class="label label-success">In stock</span></td>
                        <td class="price"><span>$ <?php echo $key['item_single_amount']; ?></span></td>
                        <td class="qty">
                            <input class="form-control input-sm" type="text" value="<?php echo $key['item_quant']; ?>" disabled readonly />
                            <a href="javascript:void(0);" onclick="vpb_add_to_cart('<?php echo strip_tags($key['item_name']); ?>','<?php echo strip_tags($key['pid']); ?>','<?php echo strip_tags($vpb_get_productdata['min_order_quantity']); ?>','<?php echo strip_tags($key['item_single_amount']); ?>','<?php echo $uskid; ?>','add');"><i class="fa fa-caret-up"></i></a>
                            <a href="javascript:void(0);" onclick="vpb_dcre_from_cart('<?php echo strip_tags($key['item_name']); ?>','<?php echo strip_tags($key['pid']); ?>','<?php echo strip_tags($vpb_get_productdata['min_order_quantity']); ?>','<?php echo strip_tags($key['item_single_amount']); ?>','<?php echo $uskid; ?>','del');"><i class="fa fa-caret-down"></i></a>
                        </td>
                        <td class="price">
                            <span>$ <?php echo($key['item_price']); ?></span>
                    ...
                    <?php $keycount++;  ?>        
                    <?php } } else { echo "<tr> <td> No Item Available in My Cart </td> </tr>"; } ?>
        ...
        else 
        {
            ...
            if(isset($_POST['item_name']) && !empty($_POST['item_name']) && isset($_POST['prod_id']) && !empty($_POST['prod_id']) &&  isset($_POST['quant']) && !empty($_POST['quant']) && isset($_POST['item_price']) && !empty($_POST['item_price']))
            {
                ...
                if(empty($uskid)) {
                    ...
                    $vpb_check_items = mysqli_query($con,"select * from `products_added` where `item_added` = '".mysqli_real_escape_string($con,$_POST['item_name'])."' and `customer_Id` = '".mysqli_real_escape_string($con,$uskid)."' and `is_purchased` = 0");
                }
