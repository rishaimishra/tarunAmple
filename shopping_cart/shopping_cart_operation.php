...
if (isset($_POST['page']) && !empty($_POST['page'])) {
                ...
                while ($key = mysqli_fetch_assoc($vpb_check_itemsdata)) {
                    ...
                    */ ?>
                    ...
                    <div class="sidebar-cart-item">
                        <a href="<?php echo $baseurl . '/productdetail/' . $key['pid']; ?>" target="_blank"
                           class="product-image">
                            <img src="<?php echo cdnUrl('product_images/' . $key['pid'] . '/' . $vpb_get_imagedata['product_image']); ?>"
                                 alt="product image" class="">
                        </a>
                        <div class="product-info">
                            <a href="<?php echo $baseurl . '/productdetail/' . $key['pid']; ?>"
                               title="<?php echo strip_tags($key['item_name']); ?>"
                               class="product-name">
                                <?php echo strip_tags($key['item_name']); ?>
                            </a>
                            <div class="product-price">
                                <span>Price: <?php echo $currencySymbol; ?> <?php echo strip_tags($key['item_price']); ?></span>
                            </div>
                            <div class="product-quantity">
                                <span>Qty: <?php echo strip_tags($key['item_quant']); ?></span>
                            </div>
                        </div>
                        <div class="remove-cart-item">
                            <button class="btn-remove"
                                    onclick="remove_this_item('<?php echo $key['pid']; ?>', '<?php echo $uskid; ?>');">
    ...
    {   
    ...
    } else if ($_POST['page'] == "add_free_to_cart"){
         // echo 7;
         //  die();
        ...
        if (isset($_POST['item_name']) && !empty($_POST['item_name']) && isset($_POST['prod_id']) && !empty($_POST['prod_id']) && isset($_POST['quant']) && !empty($_POST['quant']) && isset($_POST['item_price'])) {
            ...
            if (empty($uskid)) {
                ...
                $vpb_check_items = mysqli_query($con, "select * from `products_added` where `username` = '" . mysqli_real_escape_string($con, $_SESSION["REMOTE_ADDR"]) . "' and `item_added` = '" . mysqli_real_escape_string($con, $_POST['item_name']) . "' and `is_purchased` = 0 and `is_buy_free` = $is_free_product");
                ...
                $vpb_check_items = mysqli_query($con, "select * from `products_added` where `item_added` = '" . mysqli_real_escape_string($con, $_POST['item_name']) . "' and `customer_Id` = '" . mysqli_real_escape_string($con, $uskid) . "' and `is_purchased` = 0 and `is_buy_free` = $is_free_product");
            }
            ...
            if (mysqli_num_rows($vpb_check_items) < 1) {
                ...
                //Update products_added's table to avoid repetition of a specified item and increment items quantity and amount to display new info
                if (!empty($uskid)) {
                    mysqli_query($con, "update `products_added` set `quantity` = '" . mysqli_real_escape_string($con, $item_quantity) . "', `amount` = '" . mysqli_real_escape_string($con, $item_amount) . "', `is_buy_free` = '" . mysqli_real_escape_string($con, $is_free_product) . "' where `customer_Id` = '" . mysqli_real_escape_string($con, $uskid) . "' and `item_added` = '" . mysqli_real_escape_string($con, $_POST['item_name']) . "' and `is_purchased` = 0 and `is_buy_free` = $is_free_product and id = '" . $itemid . "'");
                } else {
                    mysqli_query($con, "update `products_added` set `quantity` = '" . mysqli_real_escape_string($con, $item_quantity) . "', `amount` = '" . mysqli_real_escape_string($con, $item_amount) . "', `is_buy_free` = '" . mysqli_real_escape_string($con, $is_free_product) . "' where `username` = '" . mysqli_real_escape_string($con, $_SESSION["REMOTE_ADDR"]) . "' and `item_added` = '" . mysqli_real_escape_string($con, $_POST['item_name']) . "' and `is_purchased` = 0 and `is_buy_free` = $is_free_product and id = '" . $itemid . "'");
                }
        ...
        if (isset($_POST['item_name']) && !empty($_POST['item_name']) && isset($_POST['prod_id']) && !empty($_POST['prod_id']) && isset($_POST['quant']) && !empty($_POST['quant']) && isset($_POST['item_price']) && !empty($_POST['item_price'])) {
            ...
            if (mysqli_num_rows($vpb_check_items) < 1) {
                            ...
                            */ ?>
                            ...
                            <div class="sidebar-cart-item">
                                <a href="<?php echo $baseurl . '/productdetail/' . $key['pid']; ?>" target="_blank"
                                   class="product-image">
                                         <img src="<?php 
                                            echo 'https://amplepoints.com/product_images/' . $key['pid'] . '/' . $vpb_get_imagedata['product_image'];
                                        ?>" alt="Product Image">
                ...
                if (mysqli_num_rows($vpb_check_all_items) < 1)// If no item exist then, display a no item message to the specified user
                {
                    ...
                    // while ($row = mysqli_fetch_assoc($vpb_check_itemsdata)) {
                    //     print_r($row); // Output the entire row for testing purposes
                    //     echo "<br>";
                    // }
                    // die();
