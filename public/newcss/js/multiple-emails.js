(function( $ ){
	...
	$.fn.multiple_emails = function(options) {
		...
		if (settings.theme.toLowerCase() == "Bootstrap".toLowerCase())
		{
			...
			deleteIconHTML = '<a href="#" class="multiple_emails-close" title="Remove"><i class="glyphicon glyphicon-remove" style="font-size:16px;color:#428bca;"></i></a>';
		}
		...
		return this.each(function() {
			...
			var $input = $('<input type="text" class="multiple_emails-input text-left" placeholder="lenore@example.com" />').on('keyup', function(e) { // input
			...
			function display_email(t, dupEmailCheck) {
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
						...
						$list.append($('<li class="multiple_emails-email"><span class="email_name" data-email="' + arr[i].toLowerCase() + '">' + arr[i] + '</span><input class="selectinvitemail" type="hidden" value="' + arr[i].toLowerCase() + '"></li>')
							  .prepend($(deleteIconHTML)
								   .click(function(e) { $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
							  )
						);
